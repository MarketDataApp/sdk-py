...
- Every log line was written once per client built without a `logger=`: each one added another stream handler to the shared `marketdata.logger`. The handler is now added once, only when that logger has none, importing the package adds none, and it writes to whatever `sys.stderr` is when a line is logged (#104)
