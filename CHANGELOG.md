...
- DataFrame and JSON output under `use_human_readable=True` use the API's column names on every resource: `options.quotes()` renamed them to the model's underscores (`Expiration_Date`, `Option_Side`), and so did the empty result of every resource (`Change_Price` for the API's `Change $`). The empty CSV header uses them too. DataFrame columns take the dtype of their model field wherever no value changes: whole numbers in a price column become floats (a chain whose strikes are all whole came back as `int64`), a column of nulls gets the field's type (`float64` for numbers on pandas, the field's type on polars), narrower polars integers widen to `Int64`, and an empty frame is typed the same way instead of `float64` (pandas) or `Null` (polars) everywhere. Human-readable date columns whose name has a space (`Expiration Date`, `First Traded`, `Report Date`) are converted to datetimes like every other date column, where `options.chain()` and `stocks.earnings()` left them as epoch integers. On polars the columns follow the model's order (the request order under `columns=`), which puts `Symbol` first in the human-readable `stocks.news()` and `Date` last in the human-readable `markets.status()`. An empty and a populated result now have the same column names, order and dtypes, so they concatenate in either order on both libraries, which polars refused (#107). A human-readable answer that lacks `Change $`, `Change %` or `Surprise EPS %` raises `ParseError` instead of a bare `KeyError`. Migration: read the human-readable `options.quotes()` DataFrame and JSON by the API's names (`df["Expiration Date"]`); the `INTERNAL` models keep their field names (`quotes.Expiration_Date`)
...
- Request-scoped response metadata: `marketdata.get_meta(result)` returns a `ResponseMeta` (`status_code`, `request_id`, `rate_limits`, `responses`) on every output format; for a call made of several requests (candle chunks, option symbols, retried attempts) the credits add up, the balance is the lowest count of the newest reset window, and `status_code` / `request_id` come from the last response that could have contributed to the result. `get_meta()` also takes the exception a failed call raised, so the credits a failure was billed are readable; there `status_code` / `request_id` are those of the request the exception is about, as on the exception itself (`None` where it says `"N/A"`, and `0` / `None` when the SDK has no response for it), and an exception that is not the SDK's own keeps the rule of a successful call (#104). `ResponseMeta`, `get_meta` and `UserRateLimits` are exported from the package root (#49)
...
- A CSV empty answer that arrives with a byte order mark in front of the API's placeholder is recognised as the empty answer, on every resource and on both fan-outs. It used to read as data: the placeholder was written into the file, and a chunk or a symbol carrying one failed the whole call with `ParseError: unknown columns ['0']`. The placeholder's `""` shape, which the other formats read too, is the empty result there as well. With a mark in front, JSON returned `''`, INTERNAL and a DataFrame raised the error each resource raises for a body it cannot read, and a fan-out failed with `ParseError`. Every CSV body the SDK reads now has a leading mark taken off through one constant; inside a value it is still data. On the resources that write the body unread the file is still what arrived, mark included; `stocks.candles` and `options.quotes` re-render theirs even for a single answer, so there the mark is gone. The API is not what puts a mark there: a proxy or a spreadsheet-friendly gateway in between is (#109)
...
- Every log line was written once per client built without a `logger=`: each one added another stream handler to the shared `marketdata.logger`. The handler is now added once, only when that logger has none, importing the package adds none, and it writes to whatever `sys.stderr` is when a line is logged, or to the stream an application sets with `setStream`, `sys.stderr` included (#104)
- A level an application sets on `marketdata.logger` itself is kept: every client built without a `logger=` used to reset it to `MARKETDATA_LOGGING_LEVEL` (`WARNING` by default), so the handlers the application had attached to that logger lost its records below that level, the SDK's `INFO` lines included. The SDK's own stderr handler still takes its level from that setting, and so does the logger while nothing else has set one on it; a level set above that setting now quiets the SDK's stderr lines below it too. Two limits: a level set only on a parent logger (`marketdata`, or the root through `logging.basicConfig`) is still not inherited, and a level set equal to the one the SDK applied cannot be told apart from it, so it keeps following the setting (#104)
