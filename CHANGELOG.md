...
- Every date field of the `OutputFormat.INTERNAL` models admits `None` in its annotation: `datetime.datetime | None` for one date, `list[datetime.datetime | None]` for a column of them, and `datetime.date | None` for `MarketStatus.date`, as `OptionsExpirations.updated` already did. A `null` date in an answer reads as `None` in its own row (see Fixed), and the annotations now say so. Migration: a type checker now asks for a `None` check before a `datetime` method is called on a date field (`if quote.updated is not None: quote.updated.date()`). At run time nothing that used to return changes: an answer with a `null` date used to raise `ParseError`, or on `OptionsQuotes` shift the column (#106)
...
- A `null` date in an answer reads as `None` on `OutputFormat.INTERNAL`, in its own row, as it does on the other formats. `OptionsQuotes` dropped it, so with three symbols and the second one's `firstTraded` null the second row got the third symbol's date and the third row none; every other model refused the answer with a `ParseError` (#106)
