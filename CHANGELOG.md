...
- `StrikeFilter` builds the `strike` filter of `options.chain` from its shapes instead of from a hand-written string: `exact`, `any_of`, `between`, `at_least`, `at_most`, `above`, `below`, and `expression` for anything the others do not name, the way sdk-go and sdk-csharp give them. `options.chain(strike=...)` also takes a plain `int`, `float` or `Decimal` now, which it refused before while the public docs typed it `float`, so a strike read from an INTERNAL chain can be passed straight back. A number is sent as its digits, never as the binary expansion behind a float (#101)
