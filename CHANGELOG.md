...
- `stocks.candles()` no longer raises `KeyError` under `columns=` on DataFrame and JSON output: the chunks are merged on the columns the first one carries. `options.quotes()` merges its symbols by the same rule; a symbol lacking a column of the first used to shift the rows of every symbol after it (the next symbol's bid on its row), or raise a bare `KeyError` on the human-readable model. On both, a JSON body that is not an object, carries none of the resource's fields, or lacks a column of the first answer raises `ParseError` naming that request (#90)
