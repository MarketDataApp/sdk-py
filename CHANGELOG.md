...
- Dates on `OutputFormat.INTERNAL` are read with the API's own rule and always come back as US/Eastern datetimes. A number from 10000 up to 200000 is a spreadsheet serial of US/Eastern wall-clock time, from 200000 Unix seconds, from `1e10` milliseconds and from `1e13` nanoseconds. A number under 10000 is a relative range for the API, not a date, and raises `ParseError`. The SDK used a boundary of its own at 60000, so from April 2064 a serial would have read as 1970. A `timestamp` date (`2026-09-21`) and a spreadsheet serial are now midnight and wall-clock time in US/Eastern, where they came back as naive datetimes, and a serial is rounded to the second. The intraday `from_date` and `to_date` of `stocks.candles()` are read only when they are ISO dates or numbers the API reads as dates (a spreadsheet serial or a Unix time), which the SDK splits into one request per year: a relative range or a keyword goes to the API as it is, in one request, where `"60"` was sent as 1900-02-28 and `"yesterday"` was refused. `UserRateLimits.reset_time` takes a datetime or Unix seconds, and refuses anything else (#116). Migration: compare INTERNAL dates against aware datetimes (`pytz.timezone("US/Eastern").localize(...)`), not naive ones
...
- The CSV output of `stocks.candles()` and `options.quotes()` merges its answers a row at a time instead of holding every row of every answer: a merge of 1,000,000 rows in 10 chunks peaked at 558 MB and now peaks at 94 MB, and takes 3.0 s instead of 5.0 s. The file is the same, byte for byte, and so is every refusal (#103)
...
- The response metadata of a single-object model and of a polars frame lives in the object's own `__dict__` (as it already did on an exception) instead of a registry keyed by identity, so `get_meta()` still reads it on a `copy.copy`, a `copy.deepcopy` or a pickle of a model, where it answered `None`. `vars(model)` now shows it under `_marketdata_meta`; the fields, `==`, `repr` and `dataclasses.asdict` do not change (#105)
