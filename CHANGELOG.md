...
- **BREAKING**: with `OutputFormat.INTERNAL`, a body its model refuses raises `ParseError` on every resource, `markets.status()`, `options.expirations()`, `options.lookup()`, `stocks.news()` and `client.utilities` included: a date it cannot parse, which used to escape as a bare `ValueError`, and a key the model does not take or a column it needs that the body lacks, which used to escape as a bare `TypeError`, so an `except ValueError` or `except TypeError` around those calls no longer catches them. For `stocks.candles()` and `options.quotes()` the error names the chunk or the symbol that sent the value, with that answer's own reason. Two cases still escape as a bare built-in: a column a model reads by name missing from the body (`Change $` and `Change %` of the human-readable `stocks.prices()` and `stocks.quotes()`, `Surprise EPS %` of the human-readable `stocks.earnings()`, `updated` and `Date` of `options.strikes()`) is a `KeyError`, and a body that is not an object of columns is #110 (#50)
