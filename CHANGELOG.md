...
- The API renders the CSV empty answer as a `200` with a placeholder body (`0` and an empty cell) instead of a `404` (MarketData-App/api#422); every resource used to write that placeholder into the CSV file. The SDK now recognises it as the empty answer to a CSV request: a header-only file, and a chunk or symbol left out of the fan-out merge. The same bytes answering a JSON request are a broken body, not an empty answer (#89)
