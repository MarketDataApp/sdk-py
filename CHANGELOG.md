...
- DataFrame dates under `DateFormat.TIMESTAMP` and `DateFormat.SPREADSHEET` are the US/Eastern datetimes the default format gives, on pandas and polars. polars left `timestamp` datetimes as strings, a `timestamp` date (`2026-09-21`) landed at 20:00 the day before on both, and a spreadsheet serial, which the API writes as US/Eastern wall-clock time, was read as UTC, four or five hours early. A serial now reads to the second, without the float noise pandas kept (#143)
