...
class BaseResource:
    ...
    def _validate_user_universal_params(
        ...
        # The API applies `columns` to the answer it sends, and drops `s` from
        # it as soon as the parameter is given, so a filtered answer is missing
        # fields the INTERNAL models require. That format asks for the whole
        # answer instead; the others pass the filter through (#23, #34).
