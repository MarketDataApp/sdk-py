...
The adapter is built once per call and reads the service status once, so an outage answers a fan-out with one ERROR line rather than one per symbol per wait. The cost of the per-request scope is that a terminal error on one request waits for the siblings' ladders before it surfaces: with the default three retries, about 7 seconds of backoff, or up to four 99-second HTTP timeouts if the siblings hang. Cancelling the pending requests instead is #98.
