...
- **A failed call is billed too**: the responses it did get, the healthy requests of a fan-out and every retried attempt consumed credits. The merged `ResponseMeta` is attached to the exception as well, so `get_meta(exc)` reports what a failure cost. A call that never reached the API (the pre-flight check) has nothing to report and carries nothing.
