...
- **One attach point, one read point**: `api_error_handler` opens a `ContextVar` scope for the call, `_make_request` records every response into it (the fan-out resources hand the scope to their worker threads with `contextvars.copy_context().run`), and the merged `ResponseMeta` is attached to the result; `get_meta()` reads it back. Lists, dicts and CSV paths become thin subclasses (`isinstance` against `list`, `dict`, `str` still holds), pandas frames use `DataFrame.attrs["marketdata"]`, and polars frames, single-object models and exceptions keep it in their own `__dict__`, so a copy, a deep copy or a pickle of a model keeps it too. An object with no `__dict__` is remembered by weak reference until it is collected; one that takes no weak references either carries nothing, which is logged at DEBUG. `None` (a single-object endpoint with no data) carries nothing.
- **Aggregation**: `credits_consumed` adds up over every response (they were all billed, whichever window billed them), `credit_limit` and `reset_time` come from the newest reset window and `credits_remaining` is the lowest count seen **within that window**, `status_code` and `request_id` from the last response whose status is usable (`VALID_STATUS_CODES`, the same list the fan-outs filter on), so a symbol or chunk that answered `no_data` and was dropped from the merge never labels the call nor lends it the request id a caller would quote to support; with no usable response the last one stands. Scoping the balance to the newest window is what keeps a call whose retries cross a reset from reporting a pre-reset count next to the new window's `reset_time`, a state that never existed on the wire; it is the same ordering rule `RateLimitTracker` applies. The status cache refresh is bookkeeping and is never part of a result.
- **A failed call is billed too**: the responses it did get, the healthy requests of a fan-out and every retried attempt consumed credits. The merged `ResponseMeta` is attached to the exception as well, so `get_meta(exc)` reports what a failure cost. Its `status_code` and `request_id` are those of the request the exception is about, read from the response an SDK HTTP exception carries (`0` and `None` when the SDK has no response for that request: no answer, or a body that does not match its `Content-Encoding`), because the responses are recorded in the order the requests finished and no rule over that list can tell which one failed (#104). An exception that is not the SDK's own does not say which request caused it, and keeps the rule of a successful call. A call that never reached the API (the pre-flight check) has nothing to report and carries nothing.
